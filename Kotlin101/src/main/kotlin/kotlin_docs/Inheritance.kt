...
/**
 * To declare an explicit supertype, place the basics.kekod2022.lesson10.getType after a colon in the class header:
 ...
 * If the derived class has no primary constructor, then each secondary constructor has to initialize the base basics.kekod2022.lesson10.getType using the super keyword
 * or it has to delegate to another constructor which does.
 * Note that in this case different secondary constructors can call different constructors of the base basics.kekod2022.lesson10.getType:
 *
 ...
 * basics.kekod2022.lesson10.Shape sınıfında draw() metodu open olarak işaretlenmiştir, bu nedenle Circle sınıfında bu metodun geçersiz kılınması için override anahtar kelimesi gereklidir.
 * Eğer override ifadesi eksikse, derleyici hata verecektir.
 * basics.kekod2022.lesson10.Shape sınıfındaki fill() metodu open olarak işaretlenmemiş ise;
...
/**
 * basics.kekod2022.lesson09.A member marked override is itself open, so it may be overridden in subclasses. If you want to prohibit re-overriding, use final:
 ...
 * Properties declared on a superclass that are then redeclared on a derived class must be prefaced with override, and they must have a compatible basics.kekod2022.lesson10.getType.
 ...
 * Bir üst sınıfta tanımlanan ve ardından bir alt sınıfta yeniden tanımlanan özellikler, override anahtar kelimesi ile belirtilmeli ve uyumlu bir tür (basics.kekod2022.lesson10.getType) kullanmalıdır.
 */
...
class FilledRectangle1 : Rectangle1() {
    ...
    inner class Filler {
        ...
        fun drawAndFill() {
            super@FilledRectangle1.draw() // Calls basics.kekod2022.lesson10.Rectangle's implementation of draw()
            fill()
            println("Drawn a filled rectangle with color ${super@FilledRectangle1.borderColor}") // Uses basics.kekod2022.lesson10.Rectangle's implementation of borderColor's get()
        }
    }
}
...
class Square() : Rectangle2(), Polygon2 {
    ...
    override fun draw() {
        super<Rectangle2>.draw() // call to basics.kekod2022.lesson10.Rectangle.draw()
